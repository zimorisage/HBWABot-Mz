...
const kumtin = moment.tz('Asia/Kolkata').format('DD/MMYYYY');
const kumzat1 = moment.tz('Asia/Kolkata').format('YYYY');
const vawiinni2 = moment.tz('Asia/Kolkata').format('dddd');
const darkarr2 = moment().tz('Asia/Kolkata').format('h:mm A');
...
case 'allmenu': { 
let BotHmangTu = ownernumber + '@s.whatsapp.net'
let me = m.sender
...
┃≡│toanime 
┃≡│tozombie
┃≡│openai [Mizo Language]
┃≡│gpt [English Language ]
┃≡│ai [Mizo language]
┃≡│ai2 [ English Language ]
┃≡└───────────┈ 
╰══════════════𖡛`]
...
if (!m.quoted) return replyherbertstyle('Message delete tur a awm chuan delete tiin reply rawh')
...
const me = m.sender;
await HBWABotMz.sendMessage(m.chat, { text: mizotranslation2, mentions: [me] }, { quoted: m });
} else {
...
HBWABotMz.sendMessage(m.chat, { document : { url : baby1[0].link}, fileName : baby1[0].nama, mimetype: baby1[0].mime }, { quoted : m })
}
break
case 'google': {
...
if (checkrawh.length == 0) return replyherbertstyle(`WhatsApp khawih te number chiah rawn dah rawh!!!`)
prem.push(premTurNb)
fs.writeFileSync('./asset/database/premium.json', JSON.stringify(prem))
replyherbertstyle(`A number ${premTurNb} hi premium a ni ta e`)
break
case 'delprem':
if (!HerbertTheCreator) return m.reply(mess.owner)
if (!args[0]) return replyherbertstyle(`Use ${prefix+command} number\n_🤖Kha tiang ringawt loh khan tiang hian tih tur_\n*⟨Entir nan :* ${prefix + command} 918416093656`)
...
case 'pornhub':{
if (!q || !q.includes("|")) {
return replyherbertstyle(`_🤖Kha tiang ringawt loh khan tiang hian tih tur_\n*⟨Entir nan :* ${prefix + command} HBWA | Bot`)
}
await makerreact()
inilogo4 = args.join(" ")
inilogo9 = args.join(" ")
...
HBWABotMz.sendMessage(m.chat, { document: { url: url }, fileName: filename, mimetype: 'application/zip' }, { quoted: m }).catch((err) => replyherbertstyle(mess.error))
break
...
*Tunah hian dar ${darkarr2} a ni mek a*`
...
if (!text) return replyherbertstyle(`Kha tiang ringawt loh khan, tiang hian word nen rawn dah rawh\n\n*⟨Entirnan :* ${prefix + command} Hello World`)
let texttts = text
...
HBWABotMz.sendMessage(m.chat, { image: { url: result }, caption: '⭔ Media Url : '+result }, { quoted: m })
